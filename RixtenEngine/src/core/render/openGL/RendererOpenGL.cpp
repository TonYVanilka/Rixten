...
RendererOpenGL::RendererOpenGL() : VAOs(12) /*refactor in future*/ {
}
...
MeshGPU RendererOpenGL::createMesh(
    ...
    glBufferData(GL_ARRAY_BUFFER, vertSize * vertLayout.stride, vertices, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, idxSize * sizeof(uint32_t), indices, GL_STATIC_DRAW);

    uint8_t location = 0;
    uint64_t offset = 0;

    for(int i = 0; i < vertLayout.attributeCount; i++) {
        const Attribute& attribute = vertLayout.attributes[i];
        glVertexAttribPointer(
            location,
            attribute.count,
            getVertexLayoutType(attribute.type),
            attribute.normalize,
            vertLayout.stride,
            (void*)static_cast<uintptr_t>(attribute.offset));
        int erroe = glGetError();
        if (erroe) LOG_ERROR(erroe);
    ...
    return MeshGPU{uint8_t(VAOs.size() - 1), vbo, ebo, uint32_t(idxSize)};
}
...
void RendererOpenGL::Draw(
    ...
    if(mesh.ibo == 0) glDrawArrays(GL_TRIANGLES, 0, mesh.indexCount);
    else glDrawElements(GL_TRIANGLES, mesh.indexCount, GL_UNSIGNED_INT, 0);
}

GLenum RendererOpenGL::getVertexLayoutType(AttributeType type) {
    switch (type) {
        case AttributeType::byte_type: return GL_BYTE;
        case AttributeType::float_type: return GL_FLOAT;
        case AttributeType::invalid_type: return GL_INVALID_ENUM;
        case AttributeType::max_enum_type: return GL_INVALID_ENUM;
        case AttributeType::short_type: return GL_SHORT;
        case AttributeType::unsigned_byte_type: return GL_UNSIGNED_BYTE;
        case AttributeType::unsigned_int_type: return GL_UNSIGNED_INT;
        case AttributeType::unsigned_short_type: return GL_UNSIGNED_SHORT;
